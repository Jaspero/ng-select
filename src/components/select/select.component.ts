...
export class SelectComponent implements ControlValueAccessor {
    ...
    switch (event.keyCode) {
      case 38:
        if (!this.active) {
          ...
          this.activeIndex = this.activeIndex > 0 ? this.activeIndex - 1 : this.filteredSelection.length - 1;
        }
      ...
      case 40:
        if (!this.active) {
          ...
          this.activeIndex = this.activeIndex < this.filteredSelection.length - 1 ? this.activeIndex + 1 : 0;
        }

        return;

      case 13:
        if (this.activeIndex === null || !this.filteredSelection.length) {
          return;
        }
  ...
  filterHandler() {
    if (this.search === '') {
      ...
      this.filteredSelection = this.formatedSelection
        .filter(value => this.matchFromStart ?
          value[this.key].indexOf(this.search) === 0 :
          value[this.key].indexOf(this.search) !== -1
        );
    }
  }
    ...
    if (value !== undefined) {
      ...
      if (this.isMulti) {
        this.selected.forEach((item: any) => {
          const index = this.formatedSelection.findIndex(i => i[this.key] === item[this.key]);
